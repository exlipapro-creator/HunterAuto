...
async function startServer() {
  ...
  // image origin — required by the How-to-Reach-Us access map (Leaflet).
  // Routing traffic does NOT need connect-src here: route requests go through
  // our own /api/v1/directions endpoint ('self').
  // Permissions-Policy: geolocation=() — browser Geolocation is fully
  // disabled: the app is fully passive about location (the How-to-Reach-Us
  // access map reads no position; customers hand off to Google Maps for
  // navigation). Camera/microphone likewise fully disabled.
